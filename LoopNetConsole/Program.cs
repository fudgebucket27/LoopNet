...
var validSince = DateTimeOffset.UtcNow.ToUnixTimeSeconds();
var validUntil = DateTimeOffset.UtcNow.AddDays(7).ToUnixTimeSeconds();
var redpacketMintResponse = await loopNetClient.PostMintRedPacketAsync(validSince, validUntil, RedPacketType.Common, RedPacketViewType.TARGET, RedPacketAmountType.AVERAGE, 1, 1m, 2m, "test", "LRC");
Console.WriteLine("Nft red packet minted: " + JsonConvert.SerializeObject(redpacketMintResponse, Formatting.Indented));
var addressesToTarget = new List<string> { "0x991B6fE54d46e5e0CEEd38911cD4a8694bed386A", "0x37ea02537f3a7a7ffc221125245905be3d5423e6" };
...
Console.WriteLine("Nft red packet sent to targets:" + JsonConvert.SerializeObject(redpacketTargetResponse, Formatting.Indented));
